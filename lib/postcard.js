...
async function loadFonts() {
  ...
  // Fraunces normal (no itálica) se suma para el título de la foto — antes
  // solo se cargaba la itálica porque todo el bloque título+descripción
  // usaba el mismo estilo. Peso 300, igual que .photo-title en el sitio.
  const [frauncesNormal, frauncesItalic, mono, signature] = await Promise.all([
    fetchGoogleFontTTF('Fraunces:wght@300'),
    fetchGoogleFontTTF('Fraunces:ital,wght@1,400'),
    fetchGoogleFontTTF('JetBrains+Mono:wght@400'),
    fetchGoogleFontTTF('Alex+Brush'),
  ]);
  fontCache = [
    { name: 'Fraunces', data: frauncesNormal, weight: 300, style: 'normal' },
...
async function renderPostcard({ title, coordsText, place, editionNumber, gift }) {
  ...
  // Muchos títulos siguen la convención "Nombre propio / descripción corta"
  // (ver .photo-title/.photo-desc en index.html, mismo criterio acá) — se
  // separa en dos líneas con tratamiento distinto en vez de mostrarlas
  // pegadas con la barra, para que el nombre no se pierda.
  const slashIdx = title.indexOf(' / ');
  const mainTitle = slashIdx === -1 ? title : title.slice(0, slashIdx);
  const shortDesc = slashIdx === -1 ? '' : title.slice(slashIdx + 3);
      ...
      props: {
        ...
        children: [
            ...
            : []),
          {
            type: 'div',
            props: {
              style: { fontFamily: 'JetBrains Mono', fontSize: 17, letterSpacing: 1, color: MID, marginTop: 18 },
              children: coordsText || '',
            },
          },
          {
