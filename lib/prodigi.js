 ...
 * @param {string} [p.postcardUrl] - URL de la tarjeta A6 (lib/postcard.js) que va adentro del sobre
 * @param {Object} p.recipient - { name, email, address:{line1,line2,postalOrZipCode,countryCode,townOrCity,stateOrCounty} }
 */
async function createOrder({ merchantReference, sku, materialId, imageUrl, postcardUrl, recipient }) {
  ...
  if (postcardUrl) {
    // OJO: branding.postcard lleva un "url" plano, NO "assets" (eso es solo
    // para items[].assets). Confirmado contra la doc oficial de Prodigi —
    // https://www.prodigi.com/print-api/docs/reference/
    body.branding = { postcard: { url: postcardUrl } };
  }
